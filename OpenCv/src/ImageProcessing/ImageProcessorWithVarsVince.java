...
public class ImageProcessorWithVarsVince {
	 ...
	 * 
	 * @param filePath
	 *            The path where file is located. Can be changed later to
	...
	public ImageProcessorWithVarsVince(String videoPath)
		...
		createToolbars();
		Size frameSize = new Size(800, 700);
	 ...
	 */
	private void processImage(frame cannyoutput, frame erodedoutput,
			frame dilatedoutput) {
		...
		Mat emptyImage = new Mat(image.size(), Core.DEPTH_MASK_8U, new Scalar(
				0, 0, 0));
		// Mat Emptyimage1 = new Mat(image.size(),Core.DEPTH_MASK_8U,new
		// Scalar(255,255,255));

		Mat canny_output = new Mat(image.size(), Core.DEPTH_MASK_8U);
		Imgproc.Canny(grayImage, canny_output, cannyThreshold,
				2 * cannyThreshold);
		...
		// HoughCircles(dilatedImage.clone(), image.clone(),
		// emptyImage.clone());
	...
	private void findContours(Mat canny_output, Mat image1, Mat emptyImage) {
		...
		Imgproc.findContours(canny_output, notDeletedContours, new Mat(),
		...
		// }
		// else{
		// System.out.println("x:"+Math.round(starPoints.get(areas.get(p)).x)
		// +"areas:"+areas.get(p));
		// }
		// }
		...
		for (int i = 0; i < contours.size(); i++) {
			 ...
			 */
			for (int index = 0; index < notDeletedContours.get(i).toList()
					.size(); index++) {
				Core.circle(image, notDeletedContours.get(i).toList()
						.get(index), 1, new Scalar(0, 255, 255), 1);
			}
...
//						new Scalar(200, 200, 250), 3);
				 ...
				 */
					...
					// contourCenters, Core.FONT_HERSHEY_COMPLEX_SMALL, 2, new
					// Scalar(200,200,250), 3);
				}
				// for(int z=0; z < contours.get(i).toList().size();z++){
				// Core.circle(image, contours.get(i).toList().get(z), 1, new
				// Scalar(0,0,255),1);
				// }
				// ArrayList<Double> areas = new
				// ArrayList<Double>(area.keySet());
				// for(int p=0; p< areas.size(); p++){
				// Core.putText(image,""+areas.get(p), area.get(areas.get(p)),
				// Core.FONT_HERSHEY_COMPLEX_SMALL, 2, new Scalar(200,200,250),
				// 3);
				// }
				...
				// System.out.println("Points In Contour: "
				// +MatOfPoint2fApprox.toList().size());

				Point contourCenter = findCenter(MatOfPoint2fApprox.toList());

				// String Color= getColor(image1,contourCenter);
				// List<Point> returnedPoint =
				// reduceEqualPoints(MatOfPoint2fApprox.toList());
				// for(int z=0;z<returnedPoint.size();z++){
				// Core.circle(image, returnedPoint.get(z), 5, new
				// Scalar(0,0,255),5);
				// }
				...
				// 3);
				//
				// for(int index=0;index < MatOfPoint2fApprox.toList().size() ;
				// index++){
				// Core.circle(image,MatOfPoint2fApprox.toList().get(index), 2,
				// new Scalar(0,255,255),3);
				// //Core.putText(image, "C:"+i,
				// MatOfPoint2fApprox.toList().get(index),
				// Core.FONT_HERSHEY_COMPLEX_SMALL, 3, new Scalar(200,200,250),
				// 3);
				// }

				// Core.circle(image, new Point(contours.get(i).get(0,
				// 0)[0],contours.get(i).get(0, 0)[1]), 10, new
				// Scalar(255,0,0),3);
			} else {
		...
		Imgproc.drawContours(emptyImage, contours, -1, new Scalar(50, 50, 50),
				10);
		Imgproc.drawContours(emptyImage1, contoursToDraw, -1, new Scalar(50,
				50, 50), 10);
	...
	private void HoughCircles(Mat canny_output, Mat image, Mat Emptyimage) {
		...
		Imgproc.HoughCircles(canny_output, circles, Imgproc.CV_HOUGH_GRADIENT,
				1, image.rows() / 100, 150, 30, 10, 500);
		...
		for (int i = 0; i < contours.size(); i++) {
			...
			for (int index = i + 1; index < contours.size(); index++) {
				...
				if (pointsEquals(findCenter(contour1.toList()),
						findCenter(contour2.toList()), pointsEqualEpsilon)) {
	...
	/*
	 * Werkt niet. Proberen op basis van verschil tussen oppervlakte gemiddelde
	 * cirkel. Af te leiden wat een cirkel is.
	 */
	private double calculateCircleDifference(MatOfPoint contour1,
			MatOfPoint contour2) {
		double averageArea = (Imgproc.contourArea(contour1) + Imgproc
				.contourArea(contour2)) / 2;
		...
		double realArea = Math.PI * ((radius1 + radius2) / 2)
				* (radius1 + radius2) / 2;
		double difference = Math.PI
				/ 4
				* (radius1 * radius1 + radius2 * radius2 - 2 * radius1
						* radius2);
		return difference;
	}
	...
	private List<MatOfPoint> addContours1(List<MatOfPoint> contours) {
		...
		for (int i = 0; i < contours.size(); i++) {
			...
			for (int index = i + 1; index < contours.size(); index++) {
				MatOfPoint contour2 = contours.get(index);
				if (pointsEquals(findCenter(contour1.toList()),
						findCenter(contour2.toList()), 250)) {
					fuzzyContours.put(i, index);
					toRemove.add(contour2);
				}
			}
	...
	private void createToolbars() {
		...
		final JTextField h2 = new JTextField("CannyThreshold: "
				+ cannyThreshold);
		...
		final JTextField epsilonApproxTF = new JTextField("Approx Epsilon:"
				+ epsilonApprox);
		...
		JSlider blurSlider = new JSlider(JSlider.HORIZONTAL, 1, 10, blur);
		...
		JSlider cannySlider = new JSlider(JSlider.HORIZONTAL, 0, 500,
				cannyThreshold);
		...
		JSlider V_Min = new JSlider(JSlider.HORIZONTAL, 1, 20, dilatesize);
		...
		JSlider V_Max = new JSlider(JSlider.HORIZONTAL, 0, 20, dilateTimes);
