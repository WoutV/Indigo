...
public class ColorWithCalibration implements ActionListener{
	...
	private void createToolbars() {
		...
		JSlider H_Min = new JSlider(JSlider.HORIZONTAL, 0, 360, 0);
		H_Min.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent ce) {
				if (((JSlider) ce.getSource()).getValue() <= ColorWithCalibration.H_Max) {
					ColorWithCalibration.H_Min = ((JSlider) ce.getSource()).getValue();
					h2.setText("H-Min:" + ColorWithCalibration.H_Min);
				} else
					((JSlider) ce.getSource()).setValue(ColorWithCalibration.H_Min);

			}
		});
		JSlider H_Max = new JSlider(JSlider.HORIZONTAL, 0, 360, 360);
		H_Max.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent ce) {
				if (((JSlider) ce.getSource()).getValue() >= ColorWithCalibration.H_Min) {
					ColorWithCalibration.H_Max = ((JSlider) ce.getSource()).getValue();
					h1.setText("H-Max:" + ColorWithCalibration.H_Max);
				} else
					((JSlider) ce.getSource()).setValue(ColorWithCalibration.H_Max);

			}
		});
		JSlider S_Min = new JSlider(JSlider.HORIZONTAL, 0, 255, 0);
		S_Min.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent ce) {
				if (((JSlider) ce.getSource()).getValue() <= ColorWithCalibration.S_Max) {
					ColorWithCalibration.S_Min = ((JSlider) ce.getSource()).getValue();
					s2.setText("S-Min:" + ColorWithCalibration.S_Min);
				} else
					((JSlider) ce.getSource()).setValue(ColorWithCalibration.S_Min);

			}
		});
		JSlider S_Max = new JSlider(JSlider.HORIZONTAL, 0, 255, 255);
		S_Max.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent ce) {
				if (((JSlider) ce.getSource()).getValue() >= ColorWithCalibration.S_Min) {
					ColorWithCalibration.S_Max = ((JSlider) ce.getSource()).getValue();
					s1.setText("S-Max:" + ColorWithCalibration.S_Max);
				} else
					((JSlider) ce.getSource()).setValue(ColorWithCalibration.S_Max);

			}
		});
		JSlider V_Min = new JSlider(JSlider.HORIZONTAL, 0, 255, 0);
		V_Min.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent ce) {
				if (((JSlider) ce.getSource()).getValue() <= ColorWithCalibration.V_Max) {
					ColorWithCalibration.V_Min = ((JSlider) ce.getSource()).getValue();
					v2.setText("V-Min:" + ColorWithCalibration.V_Min);
				} else
					((JSlider) ce.getSource()).setValue(ColorWithCalibration.V_Min);

			}
		});
		JSlider V_Max = new JSlider(JSlider.HORIZONTAL, 0, 255, 255);
		V_Max.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent ce) {
				if (((JSlider) ce.getSource()).getValue() >= ColorWithCalibration.V_Min) {
					ColorWithCalibration.V_Max = ((JSlider) ce.getSource()).getValue();
					v1.setText("V-Max:" + ColorWithCalibration.V_Max);
				} else
					((JSlider) ce.getSource()).setValue(ColorWithCalibration.V_Max);
	...
	public  Scalar getYellowMinScalar() {
		...
		return new Scalar(Integer.parseInt(s[1]), Integer.parseInt(s[2]),Integer.parseInt(s[3]));
	}

	public  Scalar getYellowMaxScalar() {
		 String[] s = yellow.split(" ");
			return new Scalar(Integer.parseInt(s[4]), Integer.parseInt(s[5]),Integer.parseInt(s[6]));
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		case "saveBlue":
			blue = "B" + " "+ H_Min + " "+ S_Min + " "+ V_Min + " "+ H_Max + " "+ S_Max + " "+ V_Max +"\n";
			saveFile();
			break;
		case "loadBlue":
			 s = blue.split(" "); 
			...
			break;
		case "saveGreen":
			green = "G" + " "+ H_Min + " "+ S_Min + " "+ V_Min+ " "+ H_Max + " "+ S_Max + " "+ V_Max+"\n";
			saveFile();
			break;
		case "loadGreen":
			s = green.split(" "); 
			...
			break;
		case "saveRed":
			red = "R" + " "+ H_Min + " "+ S_Min + " "+ V_Min+ " "+ H_Max + " "+ S_Max + " "+ V_Max+"\n";
			saveFile();
			break;
		case "loadRed":
			 s = red.split(" "); 
			...
			break;
		case "saveWhite":
			white = "W" + " "+ H_Min + " "+ S_Min + " "+ V_Min+ " "+ H_Max + " "+ S_Max + " "+ V_Max+"\n";
			saveFile();
			break;
		case "loadWhite":
			s = white.split(" "); 
			...
			break;
		case "saveYellow":
			yellow = "Y" + " "+ H_Min + " "+ S_Min + " "+ V_Min+ " "+ H_Max + " "+ S_Max + " "+ V_Max+"\n";
			saveFile();
			break;
		case "loadYellow":
			s = yellow.split(" "); 
	...
	private void saveFile() {
		try {
			...
			fw.write(blue+green+red+white+yellow);
			fw.close();
		} catch (IOException e) {
