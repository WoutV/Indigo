...
class SymbolDetectorThread implements Runnable {
	Boolean toNotify;
	 ...
	 */
	public SymbolDetectorThread(Boolean toNotify, ArrayList<Symbol> symbolList, Scalar min,
			Scalar max, SymbolsStabalization symbolS, Integer timestamp,
			String Color) {
	...
	private void findContours(Mat dilatedImage, Mat image1, Mat emptyImage)
		...
		for (int i = 0; i < contours.size(); i++) {
			...
			Imgproc.approxPolyDP(MatOfPointTo2f, MatOfPoint2fApprox, 1, true);
		...
		for (int i = 0; i < ApproxContours.size(); i++) {
			...
			Core.circle(result, contourCenter, 4, new Scalar(255, 49, 0, 255));
			...
			Core.rectangle(result, rec.tl(), rec.br(), new Scalar(255, 0, 0));
			...
			if (isRectangle(subImage, rec, contourCenter)) {
				...
				// }
					...
					Imgproc.HoughCircles(subImage, circles,
							Imgproc.CV_HOUGH_GRADIENT, 1, rec.height,
							heartThreshold, 10, (int) (rec.height / 2.5), 500);
					// System.out.println("Found circles:" + circles.cols());
