...
\texttt{x_shifted = x_original – x_zeppelin, y_shifted = y_original – y_zeppelin}
Vervolgens doen we een draaiing volgens de hoek alpha waarmee de zeppelin gedraaid staat ten opzichte van het frame:
\texttt{x_new = x_shifted*cos(alpha) + y_shifted*sin(alpha)}
\texttt{y_new = -x_shifted*sin(alpha) + y_shifted*cos(alpha)}
Nu geven we de x- en y-bestemmingen door aan de bijbehorende PositionController, zodat via PID de juiste motor wordt bijgestuurd.
